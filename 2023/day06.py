...
def part1(times_distances_pairs):
    print(reduce(int.__mul__, map(
        lambda item: count_solutions(*item), times_distances_pairs)))
...
with open("input/day06.txt") as f:
    times_distances_pairs = [(t, d) for t, d in zip(
        *list(map(lambda l: list(map(int, l.replace('\n', '').split(': ')[1].strip().split())), f.readlines())))]
